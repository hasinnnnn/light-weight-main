...
def prepare_rsi_strategy(frame: pd.DataFrame, params: dict[str, object]) -> StrategyPreparation:
    ...
    if exit_mode == "cross_down_overbought":
        exit_signal = previous_rsi.gt(overbought_level) & prepared["rsi"].le(overbought_level)
        exit_rule_summary = (
        ...
        exit_signal = prepared["rsi"].gt(exit_rsi_level)
        exit_rule_summary = f"Exit saat RSI di atas {exit_rsi_level:.0f}."
