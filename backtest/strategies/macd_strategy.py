...
def prepare_macd_strategy(frame: pd.DataFrame, params: dict[str, object]) -> StrategyPreparation:
    ...
    macd_cross_down_signal_exit = (
        previous_macd.ge(previous_signal) & prepared["macd"].lt(prepared["macd_signal"])
    )
    macd_cross_down_zero_exit = previous_macd.ge(0) & prepared["macd"].lt(0)

    if exit_mode == "macd_cross_down_zero":
        exit_signal = macd_cross_down_zero_exit
        exit_rule_summary = (
            "Exit saat MACD turun di bawah garis nol. "
        ...
        exit_signal = macd_cross_down_signal_exit
        exit_rule_summary = (
            "Exit saat MACD cross turun di bawah signal line atau saat stop loss, take profit, "
        ...
        exit_signal = macd_cross_down_signal_exit
        exit_rule_summary = (
            "Exit saat MACD cross turun di bawah signal line. "
            "Stop loss, take profit, dan trailing stop dari parameter umum tidak aktif di mode ini."
        )
        risk_management_enabled = False
